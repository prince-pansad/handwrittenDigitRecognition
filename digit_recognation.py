...
IMG_WIDTH = 28
CLASS_N = 10  # 0-9
FINAL_USER_IMG = 'final_digits.png'
...
def proc_user_img(img_file, model):
    ...
    cv2.imwrite(FINAL_USER_IMG, blank_image)
...
def showOutputImage():
	...
	frame = Frame(win, width=600, height=500)
	frame.pack()
	frame.place(anchor='center', relx=0.5, rely=0.5)
	# Create an object of tkinter ImageTk
	#img = ImageTk.PhotoImage(Image.open(TEST_USER_IMG))
	# Create a Label Widget to display the text or Image
	#label = Label(master = frame, image = img)
	#label.pack()
	pic=ImageTk.PhotoImage(master=frame,file=TEST_USER_IMG)
	Label(frame,image=pic).pack()
	# Create an object of tkinter ImageTk
	#img1 = ImageTk.PhotoImage(Image.open(FINAL_USER_IMG))
	# Create a Label Widget to display the text or Image
	#label1 = Label(master = frame, image = img1)
	#label1.pack()
	pic1=ImageTk.PhotoImage(master=frame,file=FINAL_USER_IMG)
	Label(frame,image=pic1).pack()
	win.mainloop()
	print('Completed: ')
...
def select_file():
	...
	global TEST_USER_IMG
	
	filetypes = (('Png files', '*.png'),('Jpg files', '*.jpg'))
		
	TEST_USER_IMG = fd.askopenfilename(
			title='Open a file',
			initialdir='/',
			filetypes=filetypes
			)
...
def startProcess():
	...
	btn = Button(win, text="Start Prediction", command=start)
	btn.pack()
	win.mainloop()
	print('Completed: ')
